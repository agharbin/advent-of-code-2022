...
(def input (parse-input (slurp "input.dat")))
(def moves (interleave (cycle input) (cycle [\v])))
(def input-size (count sample))
...
(def name-seq (cycle [:dash :cross :l-shape :vertical-line :square]))
...
(defn solve [initial-world move-seq shape-seq goal]
  (loop [world initial-world
         moves moves
         shapes shape-seq
         shapes-seen 0]
    (if (or (= shapes-seen goal))
      (inc (highest world))
